...
void C_EnemyManager::Update(const Math::Vector2& pPos, const Math::Vector2& pVel, GameScene* owner) {
    ...
    // 2. 追従する敵の更新（プレイヤーの位置 pPos に向かわせる）
    for (auto& fe : m_followEnemies) {
        fe->Update(pPos); // プレイヤーの位置を渡す
    }
    ...
    // 削除フラグが立った敵の掃除
    m_followEnemies.erase(
        std::remove_if(m_followEnemies.begin(), m_followEnemies.end(),
            [](const auto& e) { return !e->GetAlive(); }),
        m_followEnemies.end());
}
