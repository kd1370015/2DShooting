...
void C_Enemy::Draw(KdTexture* tex) {
    ...
    // 1. 各種行列の作成
    Math::Matrix mScale = Math::Matrix::CreateScale(scale);
    // m_angle をラジアンに変換して Z軸（画面に対して垂直な軸）回転
    Math::Matrix mRot = Math::Matrix::CreateRotationZ(DirectX::XMConvertToRadians(m_angle));
    Math::Matrix mTrans = Math::Matrix::CreateTranslation(m_pos.x, m_pos.y, 0);

    // 2. 行列を合成 (拡大 → 回転 → 移動 の順序が重要！)
    Math::Matrix mat = mScale * mRot * mTrans;
    ...
    // 4. 描画
    Math::Rectangle srcRect = { 0, 0, 32, 32 };
    // 座標は行列(mTrans)で指定済みなので、ここでは 0, 0 を渡す
    SHADER.m_spriteShader.DrawTex(tex, 0, 0, &srcRect, &color);

    // 5. 【重要】後続の描画が回転しないよう、行列をリセット（単位行列に戻す）
