...
void C_Back::Draw()
{
    // 背景の矩形情報はループの外で作っておくと効率的です
    Math::Rectangle srcRect = { 0, 0, ScreenWidth, ScreenHeight };
    ...
    for (int i = m_activeLayerCount - 1; i >= 0; --i)
    {
        ...
        for (int j = 0; j < BackTextureNum; ++j)
        {
            ...
            // 新しい定義に合わせて修正
            // 引数: (テクスチャ, x, y, &矩形)
            // 行列で位置が決まっているため、x, y は 0 を指定します
            SHADER.m_spriteShader.DrawTex(m_tex[i], 0, 0, &srcRect);
        }
    }
}